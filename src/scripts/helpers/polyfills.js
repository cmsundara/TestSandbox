...
export default function loadPolyfills(polyfills = []) {
  ...
  if (
    ...
    (!polyfills.length || polyfills.includes('IntersectionObserver'))
  ) {
    requiredPolyfills.push(
      import(
  ...
  if (
    ...
    (!polyfills.length || polyfills.includes('picture'))
  ) {
    requiredPolyfills.push(
      import(/* webpackChunkName: "polyfill-picturefill" */ 'picturefill').then(
        ({ default: picturefill }) => {
